...
# Floe — What every AI task actually costs

**Across every model, voice, tool, and infrastructure vendor — so finance can price the next deal on actuals, protect gross margin, attribute COGS to the client and campaign, and rebill customers off real numbers instead of a blended guess.**

A **task** is any AI interaction or workflow — an agent run, an LLM/tool chain, a document or vision job, a voice call. Whatever the task, it spends across a dozen vendors: the model, retrieval, the tools, the voice and carrier legs when it's voice, the infra. Those bills arrive separately, in different units, on different days, and your finance team spends weeks stitching them together to answer one question: **what did this client, this campaign, this task actually cost us?** Floe costs every task the moment it ends, closes it to vendor actuals, tags the spend to the client and campaign, and shows you your margin per contract — so you can price the next deal on actuals instead of a blended guess, and walk into your next raise with your unit economics under control. (Floe is the join across every vendor — a token router meters only the model slice, and on a voice task that is only ~40% of the bill; on BYOK, provider dashboards report $0 — plus enforcement where Floe is in the path. One ledger, tagged by agent, task, and customer.)

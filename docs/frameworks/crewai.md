...
researcher = Agent(
    ...
    llm=FloeLLM(
        ...
        credit_key=FLOE_API_KEY,                     # authenticates + identifies your balance
        provider_key=os.environ["OPENAI_API_KEY"],   # pass-through to OpenAI, never persisted
    ),
)
...
`FloeLLM` is a thin `crewai.LLM` that routes model calls through Floe's metered proxy so token spend is debited from the **same** balance as your tool calls. That's how one ceiling covers both planes.
...
Floe holds no provider keys: your `provider_key` is supplied per request, used to reach the provider, and **never persisted**. Tokens are priced at **provider cost** (from the LiteLLM cost map) plus roughly a **5% safety buffer** — at-cost, Floe takes no margin on tokens.

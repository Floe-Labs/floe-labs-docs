...
Reads are **free** (`ledger_read` for the per-leg and by-call tools, `attribution_reports` for the client/campaign rollup — both free on every plan); the two [vendor connection](../build/vendor-connections.md) tools need **Agency** `vendor_connections` (and admin/owner to verify).

...
> **Two endpoints, two id rules — don't confuse them.** The keyless gateway (`POST /v1/chat/completions`, `/v1/embeddings`, `/v1/audio/*`) resolves an **exact catalog id** — the **fully-qualified** `provider/model` used throughout this page (a bare `gpt-4o` is rejected). The legacy, flag-gated **BYOK metered proxy** `POST /v1/llm/chat/completions` is a *different* endpoint: it takes **your own** `X-Floe-Provider-Key`, prices the call from Floe's maintained LiteLLM cost map (no catalog allowlist), and accepts the model id **with or without** a `provider/` prefix — `gpt-4o` or `openai/gpt-4o` both work, since Floe strips the prefix before pricing. It meters **both** `stream:false` and `stream:true` chat completions — a streamed response is priced off the terminal `usage` chunk (Floe injects `stream_options.include_usage`) and settled as the SSE body drains, so every call lands on your ledger even though you paid the vendor directly with your own key. Prefer the keyless gateway for new work; the BYOK proxy is documented in [Unified Ledger](../build/unified-ledger.md).

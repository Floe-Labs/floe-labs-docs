...
| `kind` | string | Yes | What the spend was: `"llm"`, `"stt"`, `"tts"`, `"telephony"`, `"avatar"`, `"sms"`, `"ocr"`, `"gpu"`, or `"tool"`. Any other value is counted in `rejected` — it is never coerced to `"tool"`, so a non-LLM leg is never mis-filed as a tool call. |
| `model_or_tool` | string | Yes | The model id (`openai/gpt-4o`), voice/vendor model, or tool identifier the cost is attributed to. |

...
| `X-Floe-Task-Type` | header | What kind of work this call is (≤64 chars, lowercased) — `claims-intake`, `invoice-match`. Stamped on every leg of the call; group by it with `by=task_type` on the rollups. See [Cost per client, campaign & task](../build/attribution.md). |

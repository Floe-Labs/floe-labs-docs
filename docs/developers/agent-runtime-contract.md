...
| `X-Floe-Customer-Id: <opaque>` | optional | End-customer tag (≤128 chars, lowercased server-side) for per-client rebilling. Falls back to the agent's `defaultCustomerId` when omitted. Attribution only — it never affects budgets. Accepted on every paid surface. If the account runs **strict attribution**, a call that resolves neither is refused `400 customer_id_required` — see [Unified Ledger → Attribute spend to an end customer](../build/unified-ledger.md#attribute-spend-to-an-end-customer). |
...
| 400 | `customer_id_required` | The account requires end-customer attribution and this call resolved none. Refused before any spend; body carries a `next` block with the remediation route | **No** | Send `X-Floe-Customer-Id`, set the agent's `defaultCustomerId` (`PATCH /v1/developer/agents/:agentId`), or switch attribution back to optional |

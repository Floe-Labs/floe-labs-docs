...
An SDR shop or a back-office biller prices per unit of work — a sequence run, a claim processed — not per minute. A `task` rule bills each distinct task id on that client's metered requests. It works in `per_unit` (with an included allotment) and in `tiered` bands, alone or next to a `cost_plus` rule — cost-plus prices the cost, per-task prices the work.

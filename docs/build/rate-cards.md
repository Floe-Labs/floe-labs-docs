...
- **No silent undercount.** If any billable request for the client in the period has **no** task id, the task count is only a lower bound. The preview still shows the numbers, with an `uncounted_usage` blocker on the option, and closing the period returns `409 uncounted_usage`. There is no tolerance threshold. A request can't be re-tagged once it's recorded, so for that period the **account owner** closes with a written reason (`actualsGateOverrideReason`, 10–500 characters): the statement then bills **only the counted tasks**, so the client is never overcharged, and the reason is recorded on the period.

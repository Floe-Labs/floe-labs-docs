...
- **Name the line — the Conversations preset.** A rule's optional `label` is what the statement line and its CSV row read. Put `"label": "Conversations"` on a per-task rule and the client sees `Conversations` instead of the default `Tasks (10 of 10 included)` — a labelled line reads exactly the label, so the included-allotment suffix drops. Tiered bands take the label too (`Conversations 1–100`, `Conversations 101–412`). It is a **name, not a unit**: the rule is still per-task, counted and gated identically. The dashboard's pricing editor offers it directly as the **Conversations (per task)** option.

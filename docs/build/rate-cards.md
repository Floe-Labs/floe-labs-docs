...
| **Tiered** | `tiered`, `unit`, `tiers[]` | Graduated bands over those same units — each band prices only the units inside it. |
...
### Tiered minutes (graduated bands)

The standard voice contract — "first 1,000 minutes at $0.12, next 4,000 at $0.10, everything after that at $0.08" — is one `tiered` rule:
...
```json
{
  ...
  "label": "voice",
  "tiers": [
    { "upTo": 1000, "ratePerUnitRaw": "120000" },
    { "upTo": 5000, "ratePerUnitRaw": "100000" },
    { "upTo": null, "ratePerUnitRaw": "80000" }
  ]
}
```

Tiers are **graduated, not volume**: each band prices only the units that fall inside it, and reaching a band never reprices the units below it. 6,000 minutes on the card above bills 1,000 × $0.12 + 4,000 × $0.10 + 1,000 × $0.08 = **$600**, not 6,000 × $0.08 = $480. The kind names the model it is rather than carrying a mode flag, because the two disagree by real money on the same usage and a card is immutable once written.

- **1 to 10 bands, ascending and non-overlapping** — each `upTo` is strictly greater than the previous one (and at most `100000000`). Each band covers the units above the previous band's ceiling through its own.
- **The last band must carry an explicit `upTo: null`** — the open-ended remainder. An omitted `upTo` is refused, not normalized, so usage can never run past the top of the card and silently bill at $0.
- **A leading band priced at `0` is an included allotment** — `{ "upTo": 1000, "ratePerUnitRaw": "0" }` gives the client 1,000 free minutes before the first paid band, which is `includedUnits` generalized to more than one break.
- **One metered rule per unit.** A card can't carry both a `per_unit` and a `tiered` rule for `audio_minute` (or two of either) — they bill the same quantity, so every minute would be charged twice, permanently, on an immutable card. Metering *different* units is fine (tiered minutes plus a per-request rate), and a retainer composes with either. The dashboard's pricing editor makes voice minutes a **One rate / Tiered bands** toggle for the same reason.

A tiered rule bills as **one line item per band that has units in it**, labelled with the range it actually covered (`voice 1–1000`, `voice 1001–2500`) — a tiered invoice has to show its own arithmetic or neither the client nor you can check it. So line items are *not* one-per-rule; a band with no units in it emits nothing.

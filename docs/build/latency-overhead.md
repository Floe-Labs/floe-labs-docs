...
- **Metric:** `floe_overhead_ms` — the wall-clock time on Floe's critical path **minus** the upstream call it wraps (tracked separately as `upstream_latency_ms`). It is the *added* time, computed per call by subtracting upstream — not gateway time measured in isolation — so no separate "direct-to-vendor" control run is needed; upstream is subtracted on every call. Settlement is excluded from the measured span (see the instrumentation note below).
- **Instrumentation point:** measured inside the gateway across the path Floe owns — authenticate → gate the spend → resolve and route the model → return the response. On the proxy path the debit is deferred until after the response is flowing; on this non-streaming keyless path the ledger settle write lands after the span closes but still before the caller is fully done, so the persisted figures are a **lower bound** that excludes the settle write.

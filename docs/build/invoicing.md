...
|---|---|---|
| `statement.csv` | label, kind, quantity, unit, **amount** | **The client.** Revenue only, unless you opted that client in below. |
| `margin.csv` | everything above **plus `cost_usd`, `cost_vendor_usd`, `margin_usd`** | **You.** Your cost basis and markup. |

These are deliberately two different files with two different name prefixes, generated by two different code paths that never touch. The failure mode is one mis-click forwarding the wrong file to a client, and a distinct filename (`floe-margin-…` vs `floe-statement-…`) is the control that survives a human in a hurry. The margin file's first line says so out loud: *NOT FOR YOUR CLIENT*. Your **margin** never appears on anything a client can see — not the statement CSV, and never on any Stripe object.

### Open-book clients — disclosing vendors and cost

Some contracts are open-book: the client signed a cost-plus deal and expects to see the cost it is plus of. The [rate card](rate-cards.md#choose-what-the-client-sees) version decides, per client, with two flags that are **off by default** — the statement above carries revenue only until you set them:

- `discloseVendorNames` appends a **`vendor`** column, naming the vendor behind each line that has one.
- `discloseCosts` appends **`cost_usd`** and **`cost_vendor_usd`** — the same blank-never-zero rule as the margin file.

Disclosed columns are **appended**, never inserted: the base seven columns keep their positions, so a default statement is byte-identical to every one issued before this existed, and a parser that reads by header keeps working either way. The same flags govern the [Stripe invoice line descriptions](stripe-connect.md#push-a-statement-as-an-invoice); `margin_usd` is disclosed by neither — your markup stays on the margin file.

**Disclosure is frozen at close, like every other number on the statement.** The close snapshots the policy of the card version in force at the end of the rated window, and a closed statement is regenerated from that snapshot forever after. Appending a card version later — even one back-dated into the window — cannot flip an issued statement from hidden to disclosed. An open period has no snapshot yet, so its preview reflects the current card.

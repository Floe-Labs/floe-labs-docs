...
### v1.24.0 — The call as a first-class object, Floe Audit, and the close checklist (September 2026)

Cost stopped being a list of legs. A **call** is now one object with one duration and one cost; invoices alone are enough to get cost per client; and closing a month is a reviewed workflow instead of a one-shot POST.

* **Calls — `GET /v1/developer/interactions`.** Legs are bound into whole calls from identifiers already in the vendor data — your task id (Floe Phone defaults it to the lowercased CallSid), Twilio CallSids, vendor per-request ids, orchestrator call ids — so a call exists even where you never instrumented one, including `vendor_pull` legs that never touched Floe. Stable `int_…` ids that survive a merge, `orderBy=cost` for the expensive tail with p50/p95/max per call, `GET /interactions/rollups?by=customer|campaign|agent|channel` with duration and **cost per minute**, and `GET /interactions/:id` for one call's legs and links. A stated `resolution` figure names a reason for every leg outside it (`no_join_identifiers`, `identifier_conflict`, `resolver_error`) — deliberately not called "coverage", which is the [Coverage Score](build/coverage-score.md)'s word for a different question. Free (`ledger_read`); rollups are Pro. The older `/actuals/calls`, which groups on `task_id`, stays wire-compatible; new consumers should read `/interactions`.
* **Floe Audit — invoices in, margin per client out (`/v1/developer/audit/*`).** The front door that asks for neither traffic nor credentials: upload a vendor invoice or usage export inline (≤ 256 KiB), map `subaccount` / `phone_number` / `project` / `assistant` identifiers to your clients, and read cost, minutes, cost-per-minute, and estimated margin per client. Coverage is the label on every figure — attributed vs known cost with a named reason for the remainder, and `unmappedIdentifiers` as the work list. Non-USD lines are counted, never FX-converted. Free (`ledger_read`); in the dashboard at **Cost audit**.
* **Close, reviewed — `GET /billing-periods/:id/preview`.** The same computation the close runs, with zero writes: the lines the statement would carry, the totals, every gate that would refuse, and the legs in the window still belonging to nobody.
* **Variance — `GET /billing-periods/:id/variance`.** Estimate → vendor actual → true-up for a closed period: the frozen snapshot, the same computation re-run live, the pending true-up the next carry tick would write, the true-ups already written (`originPeriodId`), and the open findings behind them. Findings gained `customerId` / `since` / `until` filters that reach through the leg (and follow a corrected leg to its current client).
* **Allocation rules — `/v1/developer/allocation-rules`.** Named methods that spread account-level cost (committed spend, minimums, platform fees) across clients at close: `fixed` bps shares (Σ ≤ 10000 — the remainder stays with the agency), `by_usage`, `by_units`. Allocated lines are pass-through, not pricing, and carry their method and label forever, so deleting a rule never rewrites an issued statement. Two active rules claiming one adjustment kind refuse the close (`allocation_rules_conflict`). Admin + Agency to write, open to read.
* **Attribution corrections — `POST /v1/developer/actuals/legs/:id/attribution`.** Assign a client to a leg capture couldn't tag, and the dollars actually move: the leg is superseded by a corrected copy, its stamp is sealed, and the corrected leg is re-stamped under the new client so the next close bills it. Refuses on `unknown_customer`, and on `period_closed` until you `acknowledgeReprice`. List the work with `GET /actuals/legs?attributionState=unattributed`. Admin + Pro.

→ [Calls](build/interactions.md) · [Cost audit](build/cost-audit.md) · [Client invoicing](build/invoicing.md) · [Cost per client, campaign & task](build/attribution.md)
...
* **Costs arrive on the vendor's schedule.** Some legs can be costed the moment a call ends, others only on the vendor's next-day batch — so a recent leg reads `pending`, which is expected, not an error. Voice-heavy accounts show a lower share of priced legs at launch (a property of what vendors publish, not your setup); the invoice lane closes the gap.

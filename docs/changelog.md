...
> **Current counts (today):** SDKs `floe-agent` / `floe-agentkit-actions` expose **54 actions** (30 Floe + 24 x402, incl. merchant-allowlist + Floe Inference); `@floelabs/mcp-server` exposes **80 tools**; `@floelabs/cli` ships **33 commands**; the verified x402 directory catalogs **76 services across 11 categories**. Per-version numbers in the dated entries below were accurate at the time of that release.

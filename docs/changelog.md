...
* **Sign in with email or Google.** Use a one-time email code or your Google account. There's no wallet signature, no network switch, and no wallet on screen. Your first sign-in creates your account unless one already matches — Floe resolves an existing account from the Privy identity or a linked wallet first, and signs you in to that. Signed up with MetaMask, Coinbase Wallet, or another wallet? A browser you've signed in to with a wallet before lists it in the sign-in window next to email and Google; in a new browser, choose **Use a wallet instead**.

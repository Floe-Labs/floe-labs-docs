...
| x402-router | ForgeMesh Labs | $0.001 | GET | unverified |
...
**Provider:** [ForgeMesh Labs](https://router.forgemesh.io)
**Endpoint:** `GET https://router.forgemesh.io/api/services`
**Price:** $0.001 USDC per call · Base mainnet
**Floe compatible:** Yes

> Search and filter x402 services
...
curl -X POST https://credit-api.floelabs.xyz/v1/proxy/fetch \
  -H "Authorization: Bearer $FLOE_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"url": "https://router.forgemesh.io/api/services", "method": "GET"}'
```

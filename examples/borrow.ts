 ...
 *
 * ⚠️  This is the self-custody example, which signs every on-chain action
 *     from your own PRIVATE_KEY. Most agent developers should NOT use this
 *     path — the managed-wallet flow handles signing, gas, and key custody
 *     server-side and is exposed through a one-line `floe-agent register` +
 *     `floe_…` API key. See docs/getting-started/quickstart.md and
 *     docs/developers/self-custody.md for when this advanced flow is the
 *     right choice.
 *

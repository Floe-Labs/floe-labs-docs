...
`floe-labs-docs` is the **public documentation** for [Floe](https://dev-dashboard.floelabs.xyz) — COGS, attribution and margin for AI: what every AI **task** actually costs, across every model, voice, tool and infrastructure vendor. It is a **GitBook-hosted Markdown site** synced from `main`: there is no build step, no bundler, no test suite. Merging Markdown to `main` publishes it. The repo also ships two agent-consumable artifacts (`llms.txt`, `openapi/floe-api.yaml`) and is the source of record for the x402 vendor directory (`x402-directory/entries/*.json`).
...
README.md                  Homepage ("Floe — What every AI task actually costs")
...
Floe tells AI companies **what every AI task actually costs** — across every model, voice, tool and infrastructure vendor — on one ledger, tagged by client and campaign, so they can see margin per contract; budgets are enforced where Floe is in the path. A **task** is any AI interaction or workflow: an agent run, an LLM/tool chain, a document or vision job, a voice call. Voice is one task type with the deepest tooling today — not the whole category. Walletless onboarding (no crypto), fiat funding, and one API key that pays any vendor API — x402 vendors through the payment proxy (`/v1/proxy/fetch`) and LLM/voice through the keyless Floe Inference gateway (`/v1/chat/completions`, `/v1/embeddings`, `/v1/audio/*`, marketplace shim; the older BYOK metered proxy is `/v1/llm/chat/completions`) — from a prepaid balance, governed by programmable server-side spend controls. Floe Inference, the LLM proxy, spend controls, value-aware caps, quality throttle, budget-aware routing, Floe Phone, and the coverage score are **live** — describe them as live; never add "feature-flagged" or "coming soon" hedges to them.

**Primary audience: AI companies** running agents, workflows and voice — including voice operators (Vapi / Retell / Bland / Pipecat / LiveKit / CrewAI), the segment with the deepest tooling today. The through-line is "one key for the whole AI bill — and know what every task actually costs". Lead every quickstart and guide with that job-to-be-done. **Voice-specific pages stay voice-led** — the voice stack, orchestrators, Floe Phone, the voice vendor pages and the voice model catalogue are correctly scoped, not stale positioning; do not de-voice them. Keep guides developer-focused: the cost-truth / margin framing is the positioning lead (homepage, overviews), not a reason to turn a setup guide into a finance pitch.
